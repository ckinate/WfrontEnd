...
export class DateTimeService {
    ...
    getStartOfMonthPlusMonth(daysFromNow: number): DateTime {

        let date = this.getDate();
        let newDate = this.plusMonth(date, daysFromNow);
        return this.getEndOfMonthForDate(newDate);
    }
    ...
    getEndOfMonthForDate(date: DateTime | Date): DateTime {
        if (!date) {
            return date as DateTime;
        }

        if (date instanceof Date) {
            return this.getEndOfMonthForDate(this.fromJSDate(date));
        }
    ...
    plusMonth(date: DateTime, dayCount: number): DateTime {
        return date.plus({ months: dayCount });
    }
