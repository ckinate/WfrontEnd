...
import { JsonClaimMapDto, MessagingServiceServiceProxy, SendTestEmailInput, SettingScopes, TenantSettingsEditDto, TenantSettingsServiceProxy } from '@shared/service-proxies/service-proxies';
...
export class EmailSettingComponent extends AppComponentBase implements OnInit {
  ...
  constructor(injector: Injector,  private _tenantSettingsService: TenantSettingsServiceProxy,
    private _message: MessagingServiceServiceProxy,
  ...
  if (this.initialEmailSettings !== JSON.stringify(this.settings.email)) {
      this.message.confirm(
          ...
          isConfirmed => {
            if (isConfirmed) {
              this.isLoading = true;
              this._message.sendTestMail(this.testEmailAddress, "Test Mail", "Hi, this is a test mail").subscribe(result => {
                this.isLoading = false;
                      this.message.success(this.l('TestEmailSentSuccessfully'));
              }, error => {
                this.isLoading = false;
